...
public class YoutubeFilter extends GenericFilter {
        ...
        switch (downloader.getDownloaderId()) {
            case YT_DLP -> {
                switch (typeEnum) {
                    ...
                    case SUBTITLES -> {
                        if (config.getYtDlpSettings().isDownloadAutoGeneratedSubtitles()) {
    ...
    public boolean canAcceptUrl(String url, GDownloader main) {
        return !isYoutubeChannel(url) || isYoutubeChannel(url)
            && main.getConfig().getYtDlpSettings().isDownloadYoutubeChannels();
    }
